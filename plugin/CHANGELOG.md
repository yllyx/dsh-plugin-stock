...
### 修复 - 投资建议从"通用文案"到"具体板块+龙头"（核心升级）
- 修复个股匹配键名 bug（`code`/`stock_code` 双兼容，此前直接命中的个股被全部丢弃）
- 板块关键词库 10→148 词（复用 sentiment_keywords 十大分类词库，板块词方向跟随舆情情感）
- 新增 SECTOR_BRIDGE 桥接层：舆情板块名→东财真实板块（BK代码），经全量行业板块（500只，防涨幅排序截断）精确+包含匹配
- 投资建议接入东财实时数据：板块当日涨跌/5日动量/阶段（启动/发酵/高潮/退潮）+ 实时龙头打分（涨停/换手/量比/市值理由）
- 操作建议动态文案：`利好【半导体】（今日+2.1%，5日动量+5.3%，启动期），关注：中芯国际(+4.2%,量比2.1)`，不再是3条写死文案
- 前端弹窗：推荐龙头卡片（点击打开K线）、板块实时徽章；排除5开头基金代码误匹配
- 删除死代码 investment_advisor.py（从未被引用的重复实现）

### 新增 - 真实日历源（替换mock，用户要求"别编造"）
- **百度股市通经济日历**（AKShare 生产同款免费接口）：前值/预期/公布值/星级/国家，支持任意日期范围（含历史）；附A股财报披露日历
- 重大事件标题加权校准（百度星级区分度不足：非农仅2星→标题匹配提至95）
- **FOMC 2026 修正为美联储官方日历**（原编造日期8错5：6/16→6/17、7/27→7/29、9/21→9/16、11/03→10/28、12/14→12/09）；未公布年份不再编造
- 规则库降级为兜底：不定期会议（政治局/国常会/OPEC等）标记 `is_estimated`"预计"徽章；百度真实事件优先覆盖同日推测事件
- **事件→板块映射**（EVENT_SECTOR_MAP）：21类事件×受影响板块（CPI→黄金/消费、FOMC→黄金/券商/银行/地产...），写入 related_sectors 并解析 BK 代码，前端展示板块标签

### 新增 - 自动进化闭环（evolution_loop，每日盘后16:30自动执行）
- **事件回填**：已发生事件→板块实际涨跌幅（盘中事件当日反应/盘后事件次日反应）→历史影响库+actual_result，含冷启动（首拉过去14天历史事件）
- **关键词自动学习**：高置信度（频率≥10且样本≥5）新词自动进词库，其余留手动
- **预测落库+验证**：投资建议生成时对每板块记录方向预测，2天后拉实际行情自动判定正误，汇总准确率
- **日历自动刷新**：未来覆盖不足20天或历史样本不足时自动重拉
- 新端点：POST /api/evolution/run?task=backfill|learn|verify|calendar、GET /api/evolution/status
- 前端：关键词面板进化统计条（历史样本/预测准确率/已学词）+「⚡立即进化」按钮；舆情点击埋点激活用户偏好学习

### 修复（其他）
- `cleanup_old_events` 引用不存在的 `self.conn`（端点必挂）
- 日历 stale 判断：未来事件日期差为负数导致永不过期
- KeywordSuggestion 为 dataclass，进化任务按 dict 取值报错
- event_calendar 表新增 source/is_estimated/related_sectors 列（存量库自动 ALTER 兼容）

## [0.4.0] - 2026-08-25

### 新增 - 🌐 舆情联动监控系统（第 8 个 Tab + 40+ API + 19 个后端模块）

**舆情采集与过滤**
- 三数据源实时抓取（5 分钟循环）：东方财富 7×24 全球快讯、新浪财经滚动新闻、美联储官方 RSS（标准库解析，零新增依赖）
- 智能过滤引擎：200+ 核心关键词库（美国经济数据/中美贸易/国内政策/板块/黑天鹅 7 大类）+ 重要性评分（60 分门槛、时效加成）+ 情感标签
- 用户自定义关键词（增删改/股票关联词/黑名单）+ AI 推荐关键词（高频新词分析、接受/拒绝反馈）

**舆情-板块-个股关联**
- 关键词→板块映射（利率/经济数据/贸易/行业政策 4 大类映射，含影响方向与逻辑说明）
- 个股直接命中（公司名/代码识别）+ 板块龙头补充推荐
- 💡 投资建议弹窗：舆情信息 / 相关板块（利好利空+相关度）/ 相关个股 / 影响分析与操作建议

**事件日历**
- 规则库生成未来事件（非农/CPI/PCE/GDP/FOMC/国常会/政治局/财报季等，精确到分钟，EST↔CST 自动换算）
- 修复 pytz 时区冲突（localize 遇 aware datetime 全量失败）与 day_range 区间重复生成（每月一次）
- 入库字段映射兼容 + 重复生成去重

**历史影响预测与个性化**
- 历史影响数据库（事件×板块样本）+ 概率预测模型（环境修正因子）+ 风险情景分析
- 持仓优先关联、用户偏好学习、个性化推送（WebSocket 实时推送 + 桌面通知）

### 修复
- `SentimentDB` 缺失 `get_news_by_id` 方法（4 个 API 端点 500）
- `main.py` 6 处引用 `get_sentiment_db` 未导入（NameError）
- SQLite 跨线程访问（keyword_learner/impact_analyzer/event_impact_analyzer 添加 `check_same_thread=False`）
- 投资建议弹窗无法关闭：CSS 类名与 K 线加载态旧样式冲突（旧样式 `pointer-events: none` 生效），弹窗类名整体改为 `dsh-stock-adv-*`
- 舆情推送函数残留旧代码导致 `high_importance` NameError

### 已知限制
- 东财原公告接口已失效（200/0 字节），已切换 7×24 快讯接口
- 板块映射覆盖关键词命中场景（芯片/降息/关税等），普通新闻优雅降级为"暂无关联"
