...
23. **DSH 更新器安装插件报 `Invalid time value` 崩溃**：崩溃发生在 `detectMinReleaseAgeViolation`（pnpm 11.8 supply-chain 时间校验）。**对照实验已排除 registry 因素**——用 DSH 自带 pnpm 11.8 普通安装同版本（npmmirror/npmjs 双源、强制全新解析、带策略参数）全部通过；崩溃只在 DSH 更新器自己的调用上下文里发生（其内部参数组合触发 pnpm 11.8 bug，DSH 内置版无法单独升级）。**解法（实测可靠，无需等待）：退出 DSH 或直接在 web profile 手动 `"D:\Program Files\nodejs\pnpm.cmd" install 包名@版本`**（如 `pnpm install @linxin666/dsh-remote-web-ui@0.3.6`、`pnpm install dsh-plugin-stock@latest`），秒级完成，DSH 重启后正常识别；装新包（目录不存在）时甚至无需退 DSH
